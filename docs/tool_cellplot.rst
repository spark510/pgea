Cell Annotation Explorer
========================

Upload your annotation CSV to visualize UMAP cell type distributions.
...
.. raw:: html
   ...
   <script>
     ...
     async function handleFile(file) {
       ...
       stage('Reading CSV...'); document.getElementById('loading-text').textContent='Reading data...';
       pyodide.FS.writeFile('/data.csv', await file.text());
       const valid=await pyodide.runPythonAsync(`
import pandas as pd
df=pd.read_csv('/data.csv')
{'UMAP_1','UMAP_2'}.issubset(set(df.columns))
       `);
       if(!valid){log('Error: CSV must contain UMAP_1 and UMAP_2 columns','error');document.getElementById('loading').classList.remove('visible');return;}
       const cols=(await pyodide.runPythonAsync(`[c for c in df.columns if c not in ['UMAP_1','UMAP_2','conf_score','score_uncert','pred_uncert']]`)).toJs();
       const ls=document.getElementById('level-select'); ls.innerHTML='';
       for(const c of cols) ls.innerHTML+=`<option value="${c}">${c}</option>`;
       log(`Loaded: ${file.name} (${cols.length} annotation columns)`,'success');
       uploadArea.style.display='none'; document.getElementById('controls').classList.add('visible');
       await updateCellTypes(); document.getElementById('loading').classList.remove('visible');
     ...
     async function updateCellTypes(){
       ...
       const ct=(await pyodide.runPythonAsync(`sorted(pd.read_csv('/data.csv')['${lv}'].unique().tolist())`)).toJs();
       const s=document.getElementById('celltype-select'); s.innerHTML='<option value="all">All cell types</option>';
       for(const c of ct) s.innerHTML+=`<option value="${c}">${c}</option>`;
     }

     async function generatePlot(){
       const btn=document.getElementById('run-btn'); btn.disabled=true; btn.textContent='Generating...'; stage('Generating plot...');
       const lv=document.getElementById('level-select').value, ct=document.getElementById('celltype-select').value;
       try{
         await pyodide.runPythonAsync(`
import pandas as pd,numpy as np,matplotlib,io,base64
matplotlib.use('Agg')
import matplotlib.pyplot as plt; import seaborn as sns
df=pd.read_csv('/data.csv'); level_col='${lv}'; highlight='${ct}'
fig,ax=plt.subplots(1,1,figsize=(10,8))
if highlight=='all':
    cats=sorted(df[level_col].unique()); pal=sns.color_palette('tab20',len(cats)); cm=dict(zip(cats,pal))
    for c in cats:
        m=df[level_col]==c; ax.scatter(df.loc[m,'UMAP_1'],df.loc[m,'UMAP_2'],c=[cm[c]],label=c,s=6,alpha=0.7,edgecolors='none')
    ax.legend(bbox_to_anchor=(1.02,1),loc='upper left',fontsize=9,frameon=False,markerscale=3)
else:
    m=df[level_col]==highlight
    ax.scatter(df.loc[~m,'UMAP_1'],df.loc[~m,'UMAP_2'],c='#e0e0e0',s=4,alpha=0.3,edgecolors='none',label='Other')
    ax.scatter(df.loc[m,'UMAP_1'],df.loc[m,'UMAP_2'],c='#e74c3c',s=12,alpha=0.8,edgecolors='none',label=highlight)
...
img_b64=base64.b64encode(buf.read()).decode('utf-8')
     ...
     function downloadPlot(){const a=document.createElement('a');a.href=document.getElementById('plot-img').src;a.download='pangea_cellplot.png';a.click();}
     document.getElementById('level-select').addEventListener('change',()=>{if(pyodide)updateCellTypes();});
   </script>

.. seealso::

   :doc:`01_vignette_cell_annotation`
      Step-by-step tutorial for cell type annotation with PANGEApy.
